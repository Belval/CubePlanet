...
OpenglContext::OpenglContext() : m_maxFps(99999), m_fullscreen(false), m_title(""), m_lastFrameTime(0)
{
}
...
{
	m_app.create(sf::VideoMode(width, height, 32), m_title.c_str(), m_fullscreen ? sf::Style::Fullscreen : (sf::Style::Resize|sf::Style::Close), sf::ContextSettings(32, 8, 2));
}
...
void OpenglContext::CalculateFps()
{
	if ((m_Time += m_lastFrameTime) > 1.f)
	{
		m_nbFpsAvant = m_nbFps;
		m_Time = m_nbFps = 0;
	}
	m_nbFps++;
